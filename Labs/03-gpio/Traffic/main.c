...
int main(void)
{
    // Configure pedestrian traffic light
    ...
    while (1)
    {
        if (buttonPushed(&PIND, PUSH_BTN))
        {
			...
			GPIO_toggle(&PORTC, LED_GRN_C);
			GPIO_write_high(&PORTC, LED_YLW_C);
			_delay_ms(BLINK_DELAY * 3);
	        GPIO_write_low(&PORTC, LED_YLW_C);
	        GPIO_toggle(&PORTC, LED_RED_C);
	        GPIO_toggle(&PORTB, LED_RED_P);
	        GPIO_toggle(&PORTB, LED_GRN_P);
	        _delay_ms(BLINK_DELAY);
